...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? '@cerebras/cerebras_cloud_sdk/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from '@cerebras/cerebras_cloud_sdk/_shims/index'`,
