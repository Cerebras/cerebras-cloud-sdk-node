...
import { APIUserAbortError } from '@cerebras/cerebras_cloud_sdk';
import { Headers } from '@cerebras/cerebras_cloud_sdk/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Cerebras({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    test('explicit option', () => {
      const client = new Cerebras({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['CEREBRAS_BASE_URL'] = 'https://example.com/from_env';
      const client = new Cerebras({ apiKey: 'My API Key' });
    ...
    test('empty env variable', () => {
      ...
      const client = new Cerebras({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.cerebras.ai');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Cerebras({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['CEREBRAS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Cerebras({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Cerebras({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Cerebras({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['CEREBRAS_API_KEY'] = 'My API Key';
    const client = new Cerebras();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['CEREBRAS_API_KEY'] = 'another My API Key';
    const client = new Cerebras({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
